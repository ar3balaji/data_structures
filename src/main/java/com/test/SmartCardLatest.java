...
public class SmartCardLatest {
	...
	public static void main(String args[]) throws JsonParseException, JsonMappingException, IOException, ParseException, NoSuchAlgorithmException, InvalidKeyException, SignatureException, JOSEException, WriterException, InvalidKeySpecException {
		String privateKey = "MEECAQAwEwYHKoZIzj0CAQYIKoZIzj0DAQcEJzAlAgEBBCCelsasFYIW5krFvv9cN30iqHaYwF1D9By3/u2wjcv9qQ==";
		String publicKey = "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEfaV9Z/PXpnpdAF9yl7V0nhtRdF+5Beik1e+VQDaHoiPkVDaAPgAT/LpQ+CdEwxkSrbrZ4SMCK0E7fjmN7ftgdA==";
		...
		smartHealthCard.put("nbf", Instant.now().getEpochSecond());
		...
		byte[] resultJson = json.getBytes("UTF-8");
		
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		Deflater d = new Deflater();
		DeflaterOutputStream dout = new DeflaterOutputStream(os, d);
		dout.write(resultJson);
		dout.close();
		byte[] compressedJson = os.toByteArray();
		String compressedJsonStr = new String(compressedJson);
		System.out.println("Len:" + compressedJson.length);
		System.out.println(compressedJson);
		
		
		KeyFactory kf = KeyFactory.getInstance("EC");
        PKCS8EncodedKeySpec keySpecPKCS8 = new PKCS8EncodedKeySpec(java.util.Base64.getDecoder().decode(privateKey.getBytes()));
        PrivateKey privKey = kf.generatePrivate(keySpecPKCS8);

        X509EncodedKeySpec keySpecX509 = new X509EncodedKeySpec(java.util.Base64.getDecoder().decode(publicKey.getBytes()));
        PublicKey puKey = kf.generatePublic(keySpecX509);
        
		// Sign test
		JWSObject jwsObject = new JWSObject(new JWSHeader.Builder(JWSAlgorithm.ES256).keyID("CK9OG1VdSF6egmbonSGr0v85EYPjxXZhOeslo3KEuYg")
				.customParam("zip", "DEF").build(), new Payload(compressedJson));
		jwsObject.sign(new ECDSASigner((ECPrivateKey) privKey));
		String compactJWS = jwsObject.serialize();
		
        // Serialise
		String numericCompactJWS = getNumericString("shc:/" + compactJWS);
		System.out.println("Test" + compactJWS);
        System.out.println(compactJWS.length());
		System.out.println(numericCompactJWS.length());
		// Verify test
		jwsObject = JWSObject.parse(compactJWS);
		System.out.println(jwsObject.verify(new ECDSAVerifier( (ECPublicKey) puKey)));
		
	    List<QrSegment> qrSegments= new ArrayList<QrSegment>();
	    qrSegments.add(QrSegment.makeBytes("shc:/".getBytes("UTF-8")));
	    for(QrSegment qrSeg : QrSegment.makeSegments(numericCompactJWS)) {
	    	qrSegments.add(qrSeg);
	    }

	    QrCode qr0 = QrCode.encodeSegments(qrSegments, QrCode.Ecc.MEDIUM);
	    BufferedImage img = qr0.toImage(4, 10);  // See QrCodeGeneratorDemo
	    ByteArrayOutputStream os1 = new ByteArrayOutputStream();
	    ImageIO.write(img, "png", os1);
        byte[] imageBytes = os1.toByteArray();
	...
	private static Object getVaccineData(HashMap<String,Object> vaccineData, int vaccineIndex) throws ParseException {
		...
		resource.put("lotNumber",vaccineData.get("lot"));
